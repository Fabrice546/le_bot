...
Bref, écris un petit message pour dire bonjour dans le serveur !""", value="""Le_Bot, votre bot intéracitf (https://le-bot.cf) !""")
        ...
        async def on_message(message):
            ...
            if message.content == f"<@{bot.user.id}>":
                embed_mention = discord.Embed(title=f"Salut :wave:, je suis Le_Bot.", description="Fais `{}h` pour en savoir plus sur moi et mes commandes !".format(config["prefix"]), color=0xffab33)
        ...
        async def on_command_error(ctx, error):
            #Réponse si une commande n'existe pas
            if isinstance(error, CommandNotFound):
                embed_cmd_not_found = discord.Embed(title = "⛔ | Ce n'est pas une commande valide.", description="Fais `{}h` pour connaître toutes les commandes.".format(config["prefix"]), color=0xffab33)
                ...
                await ctx.reply(embed = embed_cmd_not_found)
        ...
        async def h(ctx):
            ...
            if now_id not in disabled_command:
                try :
                    embed_menu_help = discord.Embed(title = "🤖 | Option(s) à choisir :", description="Une fois l'option choisie, un nouveau message sera envoyé !", color=0xffab33)
...
▶️ Pour toutes questions, veuillez contacter le créateur de Le_Bot : \n`nathoune@le-bot.cf` par mail ou `Nathoune#3630` sur Discord.""".format(config["prefix"]), inline=True)
                    ...
                    while True:
                        ...
                        if interaction.values[0] == '⚙️ | Commandes':
                            await interaction.send(embed= embed_help_page_1, ephemeral=True)

                        if interaction.values[0] == '📋 | Mots':
                            await interaction.send(embed= embed_help_page_2, ephemeral=True)

                        if interaction.values[0] == '📂 | Informations':
                            await interaction.send(embed = embed_help_page_3, ephemeral=True)
        ...
        async def news(ctx):
            ...
            if now_id not in disabled_command:
                try :
                    embed_menu_news = discord.Embed(title = "🤖 | Option(s) à choisir :", description="Une fois l'option choisie, un nouveau message sera envoyé !", color=0xffab33)
                    ...
                    embed_news_page_1.add_field(name="`🟠 : Requiert les permissions administrateur.`", value="** **", inline=False)
...
🟡 Snaptchat       🟡 Instagram`""")
                    ...
                    while True:
                        ...
                        if interaction.values[0] == '📈 | Nouvelles commandes':
                            await interaction.send(embed = embed_news_page_1, ephemeral=True)

                        if interaction.values[0] == '⏏️ | Nouveaux mots':
                            await interaction.send(embed = embed_news_page_2, ephemeral=True)

                        if interaction.values[0] == '🤟 | Nouveau préfix':
                            await interaction.send(embed = embed_news_page_3, ephemeral=True)
        ...
        async def stats(ctx):
            ...
            if now_id not in disabled_command:
                ...
                await ctx.reply(embed=embed_stats, components = [
